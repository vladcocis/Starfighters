...

{
	...
	case PAUSED:
		...
		break;
	case DEAD:
		err = DeadMenu();   // Player has paused the game
		break;
...
ErrorType Game::DeadMenu()
{
	...
	MyDrawEngine::GetInstance()->WriteText(450, 220, L"GAME OVER", MyDrawEngine::RED);
	MyDrawEngine::GetInstance()->WriteText(600, 220, L"Final Score: " , MyDrawEngine::RED);
	MyDrawEngine::GetInstance()->WriteInt(710, 220, finalScore, MyDrawEngine::RED);

	const int NUMOPTIONS = 2;
	wchar_t options[NUMOPTIONS][11] = { L"Retry", L"Main menu" };
...
ErrorType Game::Update()
{
	...
	gameManager.Update(float(gt.mdFrameTime));
	gameManager.Render();
...
void Game::PlayerDead(int score)
{
	finalScore = score;
	ChangeState(DEAD);
}